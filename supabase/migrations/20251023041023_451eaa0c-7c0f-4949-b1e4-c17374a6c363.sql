...
BEGIN
  ...
  -- Get USER_ID of admin for reviewed_by field (not employee_id)
  ...
  IF v_employee_id IS NOT NULL THEN
    -- Pending correction - forgot to clock out
    SELECT id INTO v_attendance_id 
    FROM public.attendance_records 
    ...
    -- Approved correction - missed clock in
    SELECT id INTO v_attendance_id 
    FROM public.attendance_records 
    ...
    -- Rejected correction - both times wrong
    SELECT id INTO v_attendance_id 
    FROM public.attendance_records 
    ...
    IF v_attendance_id IS NOT NULL THEN
      INSERT INTO public.attendance_corrections (
        ...
        'Clock in/out times recorded incorrectly',
        'rejected', v_admin_user_id, NOW(),
        'CCTV footage shows actual times differ from request.',
